	...
	describe("Parameter decorator", () => {
		test.each([
			...
			class Test {
				...
				public testFunc(@Assert.Ip ip: string) {
					return ip;
				}
			}
		...
		test("GIVEN invalid ip with assertion disabled THEN does not throw", () => {
			class Test {
				@Assert.ValidateParameters
				public testFunc(
					@Assert.Ip({ assertionEnabled: false }) ip: string
				) {
					return ip;
				}
			}

			expect(() => new Test().testFunc("0.0.0")).not.toThrow();
		});
		...
		test.each(["0.0.0.0", "192.168.0.0", "178.4.234.3"])(
			...
			(value) => {
				class Test {
					@Assert.ValidateParameters
					public testFunc(@Assert.Ip(4) ip: string) {
						return ip;
					}
				}
		...
		test.each([
			...
			class Test {
				@Assert.ValidateParameters
				public testFunc(@Assert.Ip(6) ip: string) {
					return ip;
				}
			}
		...
		test("GIVEN number THEN throws", () => {
			class Test {
				@Assert.ValidateParameters
				public testFunc(@Assert.Ip ip: number) {
					return ip;
				}
			}
