	...
	describe("Parameter decorator", () => {
		test.each<string>([
			...
			class Test {
				...
				public testFunc(@Assert.Url url: string) {
					return url;
				}
			}
		...
		test.each<[string, `${string}.${string}`[]]>([
			...
			(url, allowedDomains) => {
				class Test {
					@Assert.ValidateParameters
					public testFunc(
						@Assert.Url({ allowedDomains }) url: string
					) {
						return url;
					}
				}

				expect(() => new Test().testFunc(url)).not.toThrow();
			}
		);
		...
		test.each<[string, `${string}.${string}`[]]>([
			...
			(url, allowedDomains) => {
				...
				expect(() => new Test().testFunc(url)).toThrow(
					new MultiplePossibilitiesConstraintError(
						"s.string.url",
						"Invalid URL domain",
						url,
		...
		test.each<[string, `${string}:`[]]>([
			...
			(url, allowedProtocols) => {
				class Test {
					@Assert.ValidateParameters
					public testFunc(
						@Assert.Url({ allowedProtocols }) url: string
					) {
						return url;
					}
				}
		...
		test("GIVEN invalid url with assertion disabled THEN does not throw", () => {
			class Test {
				@Assert.ValidateParameters
				public testFunc(
					@Assert.Url({ assertionEnabled: false }) url: string
				) {
					return url;
				}
			}

			expect(() => new Test().testFunc("google")).not.toThrow();
		});

		test("GIVEN a number THEN throws", () => {
			class Test {
				@Assert.ValidateParameters
				public testFunc(@Assert.Url url: number) {
					return url;
				}
			}

			expect(() => new Test().testFunc(123)).toThrow(
				new ValidationError(
					"s.string",
					"Expected a string primitive",
					123
